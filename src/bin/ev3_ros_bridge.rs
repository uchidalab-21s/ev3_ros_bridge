...
impl Ev3RosBridgeServer {
    fn new(subscriber: Arc<Mutex<Subscriber<geometry_msgs::msg::Twist>>>) -> Ev3RosBridgeServer {
        Ev3RosBridgeServer { subscriber }
    }
}
...
async fn main() -> Result<(), DynError> {
    ...
    let server = Ev3RosBridgeServer::new(Arc::new(Mutex::new(subscriber)));
